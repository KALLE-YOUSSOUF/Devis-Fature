...
if($_POST) {	
  ...
  $paymentStatus 				= $_POST['paymentStatus'];
	...
	$sql = "UPDATE facture SET order_date = '$orderDate', client_id = '$clientName', sub_total = '$subTotalValue', vat = '$vatValue', total_amount = '$totalAmountValue', discount = '$discount', grand_total = '$grandTotalValue', paid = '$paid', due = '$dueValue', payment_type = '$paymentType', payment_status = '$paymentStatus', order_status = 1 WHERE facture_id = {$orderId}";	
	$connect->query($sql);
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {		
		...
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
			...
			$orderItemTableSql = "SELECT facture_item.quantity FROM facture_item WHERE facture_item.facture_id = {$orderId}";
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {			
		$removeOrderSql = "DELETE FROM facture_item WHERE facture_id = {$orderId}";
