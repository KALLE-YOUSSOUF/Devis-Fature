...
$sql = "SELECT facture.facture_id, facture.order_date, client.client_name, client.client_prenom, client.client_adresse, client.client_tel, facture.sub_total, facture.vat, facture.total_amount, facture.discount, facture.grand_total, facture.paid, facture.due FROM facture INNER JOIN client ON facture.client_id = client.client_id WHERE facture_id = $orderId";

$orderResult = $connect->query($sql);
$orderData = $orderResult->fetch_array();

$factureId = $orderData[0];
...
$orderItemSql = "SELECT facture_item.product_id, facture_item.rate, facture_item.quantity, facture_item.total, product.product_name FROM facture_item
	INNER JOIN product ON facture_item.product_id = product.product_id 
 WHERE facture_item.facture_id = $orderId";
...
?>
 ...
 <table border="1" cellspacing="0" cellpadding="1" width="40%">
	<thead>
		<tr >
		<th width="20%;">Code :</th>
		<th width="20%;">'.'FACTURE-'.strtoupper(substr(md5(uniqid(mt_rand(), true)), 0, 4)).'</th>
		</tr>
		<tr >
		<th width="20%;">Date :</th>
...
<table border="1" width="100%;"cellspacing="1" cellpadding="2">
	...
	<tbody">
		...
		$table .= '<table border="1" width="100%;" cellpadding="7">
		
			<tr>
			    <th rowspan="7" width="60%;"></th>  
			  ...
			  <tr>
			    <td width="20%;">Remise</td>    
			    <th width="20%;">'.$discount.'</th>
			  </tr>
			  <tr>
			    <td width="20%;">SommeTotal</td>    
			    <th width="20%;">'.$grandTotal.'</th>
			  </tr>
			  <tr>
			    <td width="20%;">Montant payé</td>    
			    <th width="20%;">'.$paid.'</th>
			  </tr>
			  <tr>
			    <td width="20%;">Montant dû</td>    
			    <th width="20%;">'.$due.'</th>
			  </tr>
			<tr>
		 ...
		 <table border="1" width="100%;" cellpadding="20">
		   <tr >
			<td rowspan="2" width="50%;"> Arreter la présente facture à la somme de :<br> </td>
		   </tr>
		   <tr >
