...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			var clientName1 = $("#clientName1").val();	

			// form validation 
			if(orderDate == "") {
				$("#orderDate").after('<p class="text-danger"> La date de devis est obligatoire </p>');
			...
			if(clientName1 == "") {
				$("#clientName1").after('<p class="text-danger"> Le nom de client est obligatoire </p>');
				$('#clientName1').closest('.form-group').addClass('has-error');
			} else {
				$('#clientName1').closest('.form-group').addClass('has-success');
			} // /else
		    ...
		    if(productName[x].value == ''){	    		    	
		    	$("#"+productNameId+"").after('<p class="text-danger"> Le nom de produit est obligatoire!! </p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       
		    ...
		    if(quantity[x].value == ''){	    	
		    	$("#"+quantityId+"").after('<p class="text-danger"> Le nom de produit est obligatoire!! </p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName1) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	' <br /> <br /> <a type="button" onclick="printOrder('+response.order_id+')" class="btn btn-primary"> <i class="glyphicon glyphicon-print"></i> Imprimer </a>'+
	            	'<a href="orders.php?o=add" class="btn btn-success" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Ajouter un nouveau devis </a>'+
	...
	if(orderId) {		
		...
		$.ajax({
			...
			success:function(response) {
				...
				var mywindow = window.open('', 'Devis - Facture - Stock', 'height=400,width=600');
...
function removeProductRow(row = null) {
	if(row) {
		...
		alert('erreur! rafraîchir la page à nouveau');
	}
}
...
function getProductData(row = null) {
	if(row) {
		...
		alert('Pas de ligne !! Veuillez rafraîchir la page');
	}
} // /select on product data
